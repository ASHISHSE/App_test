...
def load_circlewise_data():
    ...
    for col in ["NDVI", "NDWI", "RAINFALL_DEV", "MAI"]:
...
# -----------------------------
# FILTER CIRCLEWISE DATA
# -----------------------------
def filter_circlewise_data(district=None, taluka=None, circle=None, year=None, month=None):
...
if generate:
    if not district or not crop:
        ...
        with tab3:
            ...
            st.subheader("📊 Combined Indicator View")
            col1, col2, col3, col4, col5 = st.columns(5)
            district_sel = col1.selectbox("Select District", sorted(circlewise_df["District"].unique()))
            taluka_sel = col2.selectbox("Select Taluka", sorted(circlewise_df.query("District == @district_sel")["Taluka"].unique()))
            circle_sel = col3.selectbox("Select Circle", sorted(circlewise_df.query("Taluka == @taluka_sel")["Circle"].unique()))
            year_sel = col4.selectbox("Select Year", sorted(circlewise_df["Year"].unique()))
            month_sel = col5.selectbox("Select Month", sorted(circlewise_df["Month"].unique()))
            filtered = filter_circlewise_data(district_sel, taluka_sel, circle_sel, year_sel, month_sel)
