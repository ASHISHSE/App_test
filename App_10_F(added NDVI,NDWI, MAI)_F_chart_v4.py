...
def load_circlewise_data():
    url = "https://github.com/ASHISHSE/App_test/blob/main/Circlewise_Data_Matrix_Indicator_2024_F_upload.xlsx"
...
# -----------------------------
# NEW FUNCTION TO CREATE COMPARISON LINE CHARTS
# -----------------------------
def create_comparison_line_charts(comparison_data, selected_level, selected_name):
    """Create line charts comparing all sub-levels within the selected level"""
...
if generate:
    if not district or not crop:
        ...
        with tab2:
            ...
            # COMPARISON CHARTS FOR SUB-LEVELS
            st.markdown("---")
            st.header("🔍 Comparison Charts - Sub-level Analysis")
            
            if not comparison_data.empty:
                # Create comparison line charts
                ndvi_comparison_fig, ndwi_comparison_fig = create_comparison_line_charts(
                    comparison_data, level, level_name
                )
                
                if ndvi_comparison_fig and len(ndvi_comparison_fig.data) > 0:
                    st.subheader("🌿 NDVI Comparison - Sub-levels")
                    st.plotly_chart(ndvi_comparison_fig, use_container_width=True)
                else:
                    st.info("No NDVI comparison data available for sub-levels.")
                
                if ndwi_comparison_fig and len(ndwi_comparison_fig.data) > 0:
                    st.subheader("💧 NDWI Comparison - Sub-levels")
                    st.plotly_chart(ndwi_comparison_fig, use_container_width=True)
                else:
                    st.info("No NDWI comparison data available for sub-levels.")
                
                # Show summary of available sub-levels
                st.subheader("📋 Available Sub-levels for Comparison")
                if level == "District":
                    talukas_available = comparison_data['Taluka'].unique()
                    if len(talukas_available) > 0:
                        st.write(f"**Talukas in {level_name} District:** {', '.join(talukas_available)}")
                    else:
                        st.info("No taluka data available for comparison.")
                
                elif level == "Taluka":
                    circles_available = comparison_data[comparison_data['Taluka'] == level_name]['Circle'].unique()
                    if len(circles_available) > 0:
                        st.write(f"**Circles in {level_name} Taluka:** {', '.join(circles_available)}")
                    else:
                        st.info("No circle data available for comparison.")
            else:
                st.info("No comparison data available for sub-level analysis.")
