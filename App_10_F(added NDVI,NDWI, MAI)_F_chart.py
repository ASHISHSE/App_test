...
if generate:
    if not district or not crop:
        ...
        # -----------------------------
        # Monthly Indicator Comparison
        # -----------------------------
        st.markdown("---")
        st.header("📊 Monthly Indicator Comparison (Categories Only)")

        # Detect indicator columns dynamically
        indicator_cols = [c for c in weather_df.columns if "Indicator-" in c]
        st.write("✅ Detected Indicator Columns:", indicator_cols)

        months = sorted({re.search(r"_(January|February|March|April|May|June|July|August|September|October|November|December)", c).group(1)
                         for c in indicator_cols if re.search(r"_(January|February|March|...|December)", c)})

        comparison_rows = []
        for month in months:
            row = {"Month": month}
            for i in range(1, 4):
                col_name = f"Indicator-{i} {month}"
                match_col = [c for c in indicator_cols if month in c and f"Indicator-{i}" in c]
                if match_col:
                    row[f"Indicator-{i}"] = weather_df[match_col[0]].iloc[0]  # pick first value
        ...
        if comparison_rows:
            ...
            st.info("No monthly indicator data found.")
