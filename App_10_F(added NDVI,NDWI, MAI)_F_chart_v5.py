...
# -----------------------------
# MODIFIED FUNCTION TO CREATE COMPARISON LINE CHARTS BASED ON CONDITIONS
# -----------------------------
def create_comparison_line_charts_modified(comparison_data, selected_level, selected_name, district, taluka, circle):
    """Create line charts based on the selection level conditions"""
            ...
            for circle_name in circles_in_taluka:
                ...
                if any(pd.notna(circle_data['NDVI'])):
                    ndvi_circle_fig.add_trace(go.Scatter(
                        ...
                        y=circle_data['NDVI'],
                        mode='lines+markers',
                        name=f'{taluka_name} - Circle: {circle_name}',
                        line=dict(width=2),
                        marker=dict(size=6)
                    ))
...
# -----------------------------
# ORIGINAL FUNCTION TO CREATE COMPARISON LINE CHARTS (for backward compatibility)
# -----------------------------
...
# -----------------------------
# MODIFIED FUNCTION FOR MAI ANALYSIS - CLUSTER COLUMN CHART
# -----------------------------
def create_mai_analysis_clustered(comparison_data, selected_level, selected_name, district, taluka, circle):
    """Create clustered column chart for MAI analysis with averages excluding 0 values"""
    if comparison_data.empty:
        return None
            ...
            if not matrix_data_rs.empty:
                ...
                if monthly_df_rs is not None and not monthly_df_rs.empty:
                        ...
                        # Condition II: District level only - show 4 charts
                            ...
                            with col1:
                                if ndvi_taluka_fig and len(ndvi_taluka_fig.data) > 0:
                                    st.plotly_chart(ndvi_taluka_fig, use_container_width=True)
                                else:
                                    st.info("No NDVI taluka average data available.")
                            with col2:
                                if ndwi_taluka_fig and len(ndwi_taluka_fig.data) > 0:
                                    st.plotly_chart(ndwi_taluka_fig, use_container_width=True)
                                else:
                            ...
                            with col1:
                                if ndvi_circle_fig and len(ndvi_circle_fig.data) > 0:
                                    st.plotly_chart(ndvi_circle_fig, use_container_width=True)
                                else:
                                    st.info("No NDVI circle data available.")
                            with col2:
                                if ndwi_circle_fig and len(ndwi_circle_fig.data) > 0:
                                    st.plotly_chart(ndwi_circle_fig, use_container_width=True)
                                else:
                    ...
                    if mai_clustered_chart:
                        st.plotly_chart(mai_clustered_chart, use_container_width=True)
                    else:
